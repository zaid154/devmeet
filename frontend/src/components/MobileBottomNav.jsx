...
const MobileBottomNav = () => {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#000000]/95 backdrop-blur-md border-t border-[#222733] py-2 px-3 flex items-center justify-around select-none">
      
      {/* 1. Swipe Tab */}
      <Link
        to="/feed"
        className={`flex flex-col items-center justify-center space-y-1 py-1 px-3 transition-colors ${
          isSwipeActive ? 'text-[#fe3c72]' : 'text-gray-400 hover:text-gray-200'
        }`}
      >
      ...
      <Link
        to="/app/explore"
        className={`flex flex-col items-center justify-center space-y-1 py-1 px-3 transition-colors ${
          isExploreActive ? 'text-[#fe3c72]' : 'text-gray-400 hover:text-gray-200'
        }`}
      >
      ...
      <Link
        ...
        className={`flex flex-col items-center justify-center space-y-1 py-1 px-3 transition-colors relative ${
          location.search.includes('tab=requests') ? 'text-[#fe3c72]' : 'text-gray-400 hover:text-gray-200'
        }`}
      >
        <svg className="w-6 h-6 fill-current" viewBox="0 0 24 24">
      ...
      <Link
        to="/chat"
        className={`flex flex-col items-center justify-center space-y-1 py-1 px-3 transition-colors relative ${
          isChatActive ? 'text-[#fe3c72]' : 'text-gray-400 hover:text-gray-200'
        }`}
      >
        <svg className="w-6 h-6 fill-current" viewBox="0 0 24 24">
      ...
      <Link
        to="/profile"
        className={`flex flex-col items-center justify-center space-y-1 py-1 px-3 transition-colors ${
          isProfileActive ? 'text-[#fe3c72]' : 'text-gray-400 hover:text-gray-200'
        }`}
      >
        {user?.profileImage ? (
          <img
            ...
            className={`w-6 h-6 rounded-full object-cover border ${
              isProfileActive ? 'border-[#fe3c72]' : 'border-gray-600'
            }`}
          />
        ) : (
          <svg className="w-6 h-6 fill-current" viewBox="0 0 24 24">
